// Adapted from React Bits <SideRays /> — plain JSX, inline styles, ogl loaded from CDN at runtime.
...
function SideRays({
  speed = 1.0, rayColor1 = '#ffaa6e', rayColor2 = '#96c8ff', intensity = 1.0, spread = 1.0,
  origin = 'top-right', tilt = 0, saturation = 1.0, blend = 0.78, falloff = 2.0, opacity = 1.0,
  className = '', style,
}) {
  const num = (v, d) => { const n = Number(v); return Number.isFinite(n) ? n : d; };
  speed = num(speed, 1); intensity = num(intensity, 1); spread = num(spread, 1);
  tilt = num(tilt, 0); saturation = num(saturation, 1); blend = num(blend, 0.78);
  falloff = num(falloff, 2); opacity = num(opacity, 1);
  ...
  useEffect(() => {
    ...
    (async () => {
      let ogl;
      try { ogl = await import('https://esm.sh/ogl@1.0.11'); }
      catch (e) { console.warn('SideRays: ogl failed to load', e); return; }
      ...
      const uniforms = {
        ...
        iSpeed: { value: speed }, iRayColor1: { value: hexToRgb(rayColor1) }, iRayColor2: { value: hexToRgb(rayColor2) },
        iIntensity: { value: intensity }, iSpread: { value: spread },
        iFlipX: { value: flipX }, iFlipY: { value: flipY }, iTilt: { value: tilt },
        iSaturation: { value: saturation }, iBlend: { value: blend }, iFalloff: { value: falloff }, iOpacity: { value: opacity },
      };
  ...
  return React.createElement('div', {
    ...
    style: Object.assign({ position: 'relative', width: '100%', height: '100%', overflow: 'hidden', pointerEvents: 'none' }, style),
  });
}
