 ...
 *
 * Submissions go to Netlify Forms under the name "newsletter".
 */
export default function NewsletterForm() {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setError(null);
    try {
      await submitNetlifyForm(NETLIFY_FORM_NAMES.newsletter, form);
      setSent(true);
    } catch {
  ...
  if (sent) {
    return (
      <div className={styles.success} role="status">
        ...
        <span className={styles.successText}>You&rsquo;re on the list.</span>
      </div>
    );
  }
